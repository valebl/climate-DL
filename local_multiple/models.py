...
class Classifier(nn.Module):
    def __init__(self, input_size=5, gru_hidden_dim=12, cnn_output_dim=256, n_layers=2, num_node_features=1):
        ...
        self.gnn = geometric_nn.Sequential('x, edge_index, edge_attr', [
            ...
            (GATv2Conv(self.gnn_node_dim, self.gnn_node_dim, aggr='mean', dropout=0.6, edge_dim=2), 'x, edge_index, edge_attr -> x'), 
            (geometric_nn.BatchNorm(self.gnn_node_dim), 'x -> x'),
            nn.ReLU(),                                                     
            (GATv2Conv(self.gnn_node_dim, self.gnn_node_dim, aggr='mean', dropout=0.6, edge_dim=2), 'x, edge_index, edge_attr -> x'),
            (geometric_nn.BatchNorm(self.gnn_node_dim), 'x -> x'),
            ...
            (GATv2Conv(self.gnn_node_dim, 1, aggr='mean', edge_dim=2), 'x, edge_index, edge_attr -> x'), # focal loss
...
class Regressor(nn.Module):
    def __init__(self, input_size=5, gru_hidden_dim=12, cnn_output_dim=256, n_layers=2, num_node_features=1):
        ...
        self.gnn = geometric_nn.Sequential('x, edge_index, edge_attr', [
            ...
            (geometric_nn.BatchNorm(self.gnn_node_dim), 'x -> x'),
            nn.ReLU(),
            (GATv2Conv(self.gnn_node_dim, 1, aggr='mean', dropout=0.6, edge_dim=1), 'x, edge_index, edge_attr -> x'),
            #(geometric_nn.BatchNorm(self.gnn_node_dim), 'x -> x'),
            #nn.ReLU()
            ])
...
class Regressor_GNN(nn.Module):
    ...
    def forward(self, encoding_batch, data_batch, num_node_features=1): # encoding_batch.shape = (batch_dim, space_dim, time_dim, 128)
        for i, data in enumerate(data_batch):
            features = torch.zeros((data.num_nodes, num_node_features + self.encoding_dim)).to(device)
            features[:,0] = data.x
