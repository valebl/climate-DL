...
#        else:
        x = torch.zeros((25, len(self.low_res), 1 + self.encodings[-1].shape[1]))
        for idx in self.low_res.unique():
            idx_mask = self.low_res == idx
            encoding = self.encodings[idx][time_index-24:time_index+1,:].unsqueeze(1).repeat(1,idx_mask.sum(),1)
...
def custom_collate_fn_gnn_temporal(batch):
