...
def write_log(text, accelerator, args, mode='a'):
    if accelerator is None or accelerator.is_main_process:
        with open(args.output_path+args.out_log_file, mode) as f:
            f.write(text)
...
def load_checkpoint(model, checkpoint_file, accelerator, net_names, args, model_name=None, fine_tuning=True):
    write_log(f"\nLoading parameters of model {model_name}.", accelerator, args) 
    ...
    for name, param in checkpoint["parameters"].items():
        ...
        for net in net_names:
            if net in name:
                write_log(f"\nLoading parameters '{name}'", accelerator, args)
...
def check_freezed_layers(model, accelerator, args):
    for name, param in model.named_parameters():
        n_param = param.numel()
        write_log(f"\nLayer {name} requires_grad = {param.requires_grad} and has {n_param} parameters", accelerator, args)
...
def train_epoch_ae(model, dataloader, loss_fn, optimizer, lr_scheduler, loss_meter, performance_meter, val_loss_meter,
        val_performance_meter, validationloader, validate_model, accelerator, args, intermediate=False, epoch=0):
...
def train_epoch_gnn(model, dataloader, loss_fn, optimizer, lr_scheduler, loss_meter, performance_meter, val_loss_meter,
    ...
    for X, data in dataloader:
        optimizer.zero_grad()
        y_pred, y = model(X, data, era5_to_gripho_list, device)
...
def train_model(model, dataloader, loss_fn, optimizer, train_epoch, validate_model, validationloader, accelerator, args,
    ...
    for epoch in range(epoch_start, epoch_start + num_epochs):
        ...
        write_log(f"\nEpoch {epoch+1} --- learning rate {optimizer.param_groups[0]['lr']:.8f}", accelerator, args)
...
def test_model_ae(model, dataloader, log_path, log_file, accelerator, loss_fn=None, performance=None):
    ...
    write_log(f"\nTESTING - loss total = {fin_loss_total if fin_loss_total is not None else '--'},"
            +f"loss avg = {fin_loss_avg if fin_loss_avg is not None else '--'}", accelerator, args)
